...
class OSE_API AssetSystem {
 public:
  ...
  // STATIC MESH
  StaticMesh* loadStaticMesh(const string& name, string path,
                             vec4 bottomExtrusion = vec4(0, 0, 0, -1),
                             vec4 topExtrusion = vec4(0, 0, 0, 1));
 ...
 protected:
  ...
  std::vector<Tetrahedron> cutPrism(vec4 a1, vec2 u1, vec4 a2, vec2 u2, vec4 a3,
                                    vec2 u3, vec4 b1, vec2 v1, vec4 b2, vec2 v2,
                                    vec4 b3, vec2 v3);
