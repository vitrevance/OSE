...
GlRenderer::GlRenderer() {
  for (std::pair<const string, StaticMesh*> it :
       AssetSystem::instance->getStaticMeshes()) {
    this->setupStaticMesh(it.second);
  }
  for (std::pair<const string, Material*> it :
       AssetSystem::instance->getMaterials()) {
    for (string textureName : it.second->textures) {
      ...
      if (texture == nullptr) {
        OSE_LOG(LOG_OSE_ERROR,
                "GlRenderer: unresolved texture <" + textureName + ">!");
        break;
      }
      if (texture->id == 0) {
...
std::endl #define printt(t) printv(t.vertex); printv(t.base_1);
printv(t.base_2); printv(t.base_3) #define printm(m) for (int MII = 0; MII < 16;
MII++) {std::cout << m[MII / 4][MII % 4] << " "; if (MII % 4 == 0) {std::cout <<
std::endl;}}
*/
    int projectionLoc =
        glGetUniformLocation(this->m_mainShader, "uMatProjection");
    ...
    for (int i = 0; i < textureId.size(); i++) {
      ...
      glUniform1i(loc, i);
      // glUniform1i(glGetUniformLocation(this->m_mainShader, ("texture" +
    ...
    // glDrawElementsInstanced(GL_TRIANGLES, mesh->isize, GL_UNSIGNED_INT, 0,
    // this->m_batch[mesh].size());
    glDrawArraysInstanced(GL_POINTS, 0, mesh->cells.size(),
                          this->m_batch[mesh].size());
  }
  glBindVertexArray(0);
}
...
Renderer::Shader GlRenderer::createShader(string shaderName) {
  string fragmentText =
      AssetSystem::instance->loadRawString(shaderName + ".frag");
  string vertexText =
      AssetSystem::instance->loadRawString(shaderName + ".vert");
  string geometryText =
      AssetSystem::instance->loadRawString(shaderName + ".geom");
...
void GlRenderer::setupStaticMesh(StaticMesh* mesh) {
  ...
  &mesh->cells[0], GL_STATIC_DRAW); glEnableVertexAttribArray(0);
  glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex), 0);
  glEnableVertexAttribArray(1);
  glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex),
  (void*)offsetof(Vertex, norm)); unsigned int MBO; glGenBuffers(1, &MBO);
  this->m_instanceBuffers[mesh] = MBO;
  glGenBuffers(1, &mesh->EBO);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mesh->EBO);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * mesh->isize,
  ...
  */
  ...
  glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, sizeof(Tetrahedron),
                        (void*)offsetof(Tetrahedron, base_2));
  glEnableVertexAttribArray(3);
  glVertexAttribPointer(3, 4, GL_FLOAT, GL_FALSE, sizeof(Tetrahedron),
                        (void*)offsetof(Tetrahedron, base_3));
  glEnableVertexAttribArray(4);
  glVertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, sizeof(Tetrahedron),
                        (void*)offsetof(Tetrahedron, uvvertex));
...
Renderer::Shader GlRenderer::createShader(string vertexText,
                                          string geometryText,
                                          string fragmentText) {
  unsigned int vertexId = glCreateShader(GL_VERTEX_SHADER);
  const char* vertexChar = vertexText.c_str();
...
void GlRenderer::setupShader() {
  ...
  {
    auto resources = cmrc::OSE::get_filesystem();
    auto file = resources.open("shaders/mainShader.vert");
    vertexText = std::string_view(file.begin(), file.end());

    file = resources.open("shaders/mainShader.frag");
    fragmentText = std::string_view(file.begin(), file.end());

    file = resources.open("shaders/mainShader.geom");
    geometryText = std::string_view(file.begin(), file.end());
  }
  ...
  for (std::pair<const string, Material*>& it : materials) {
    ...
    materialSwitch += "case " + std::to_string(m->id) + ": return material" +
                      std::to_string(m->id) + "();";
    materialText += m->text;
